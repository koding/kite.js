...
module.exports = function (options = {}) {
  const backoff = options.backoff != null ? options.backoff : {}
  let totalReconnectAttempts = 0
  const initalDelayMs = backoff.initialDelayMs != null ? backoff.initialDelayMs : 700
  const multiplyFactor = backoff.multiplyFactor != null ? backoff.multiplyFactor : 1.4
  const maxDelayMs = backoff.maxDelayMs != null ? backoff.maxDelayMs : 1000 * 15 // 15 seconds
  const maxReconnectAttempts = backoff.maxReconnectAttempts != null ? backoff.maxReconnectAttempts : 50
